...
static void fn(struct mg_connection *c, int ev, void *ev_data, void *fn_data)
{
  ...
  else if (ev == MG_EV_HTTP_MSG)
  {
    ...
    if (mg_http_match_uri(hm, "/api/sign/*") && std::string_view(hm->method.ptr, hm->method.len) == "POST")
    {
      ...
      const PKey *key = kmgr.fetch_key(split(uri, '/')[3]);
      if(!key)
        mg_http_reply(c, 404, "Content-Type: application/json\r\n", "{message:\"Key not found\"}");
      else {

        std::string_view uri(hm->body.ptr, hm->body.len);
        std::vector<uint8_t> input(uri.cbegin(), uri.cend());
        auto result = key->sign_base64(input.data(), input.size());
...
int main(int argc, char *argv[])
{
  ...
  if (argc >= 2 && std::string_view(argv[1]) == "sign")
  {
    parse_opts(argv);
    ...
    std::cout << key.sign_base64(input.data(), input.size());
  }
